...
<style>
  ...
  /* overlay “không có dữ liệu” khi cần (vẫn vẽ bằng plugin, lớp này làm nền mờ nhẹ) */
  .chart-empty{
...
{{-- Stat pills (tính trực tiếp từ dữ liệu) --}}
<div class="mb-4">
...
window.addEventListener('DOMContentLoaded', () => {
  // ====== DỮ LIỆU TỪ CONTROLLER (giữ nguyên biến cũ) ======
  ...
  const hasData = (arr)=> Array.isArray(arr) && arr.some(v => (+v||0) > 0);

  // Plugin hiện chữ “Không có dữ liệu” nếu dataset trống
  const noDataPlugin = {
    id: 'noData',
    afterDraw(chart, args, opts) {
      const any = (chart.data.datasets||[]).some(ds => (ds.data||[]).some(v => (+v||0) > 0));
      const elId = chart.canvas.id;
      const map = {
      ...
      const overlay = document.getElementById(map[elId]);
      if(!overlay) return;
      overlay.classList.toggle('show', !any);
      if(!any){
        const {ctx, chartArea} = chart;
        if(!chartArea) return; // chưa layout xong
        ctx.save();
        ctx.fillStyle = '#6b7280';
        ctx.font = '600 14px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial';
        ctx.textAlign = 'center';
        ctx.fillText(opts?.label || 'Không có dữ liệu', (chartArea.left+chartArea.right)/2, (chartArea.top+chartArea.bottom)/2);
        ctx.restore();
      }
    }
  };
  ...
  const commonTooltip = {
    callbacks: {
      label: (ctx)=>{
        ...
        const v = ctx.parsed?.y ?? ctx.parsed; // line/bar có y, doughnut là số
